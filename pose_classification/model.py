        ...
        for key in config.keys():
            if "linear" in key:
                ...
                if model_name == "pose_classifier_v2.1":    
                    module = [conv_layer, nn.ReLU(), nn.BatchNorm1d(config[key][1])]
                elif model_name == "pose_classifier_v2.2":
                    module = [conv_layer, nn.BatchNorm1d(config[key][1]), nn.ReLU()]
                else:
                    pass 
...
class PoseClassifierV2_1(BasePoseClassifier):
        ...
        for key in config.keys():
            if "linear" in key:
                ...
                module = nn.Sequential(nn.Dropout(.5),
                                       nn.Linear(config[key][0], config[key][1]),
                                       nn.ReLU())
    ...
    def forward(self, xb, device):
        xb = xb.float()
        xb.to(device)
        conv_block_list = [block for block in list(self.config.keys()) if "conv" in block]
        linear_block_list = [block for block in list(self.config.keys()) if "linear" in block]
...
class End2EndPoseClassifer(nn.Module):
    ...
    def forward(self, xb, device):
        ...
        if lying_left_indexes.size()[0] != 0:
            raw_lying_left_samples = torch.index_select(input=xb, dim=0, index=lying_left_indexes)
            lying_left_outputs = self.lying_left_model(raw_lying_left_samples, device)
        ...
        if supine_indexes.size()[0] != 0: 
            raw_supine_samples = torch.index_select(input=xb, dim=0, index=supine_indexes)
        ...
        if lying_right_indexes.size()[0] != 0:
            raw_lying_right_samples = torch.index_select(input=xb, dim=0, index=lying_right_indexes)
            lying_right_outputs = self.lying_right_model(raw_lying_right_samples, device)
        ...
        raw_pred = raw_pred.index_fill(dim=0, index=lying_left_indexes, value=1)
        raw_pred = raw_pred.index_fill(dim=0, index=supine_indexes, value=0).numpy()
